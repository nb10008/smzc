...
DWORD RTServer::HandlePrivilegeInfo(tagNetCmd *command)
{
	...
	g_pMainFrame->ShowPage( VerifyGMPrivilege(Crc32("NRTC_Start"), GetPrivilege()) && VerifyGMPrivilege(Crc32("NRTC_Close"), GetPrivilege()), 
		...
		VerifyGMPrivilege(Crc32("NRTC_UpdateGodBid"), GetPrivilege()),
		VerifyGMPrivilege(Crc32("NRTC_ReloadItemNeedLog"), GetPrivilege()));

	//g_pMainFrame->ShowPage(true,true,true,true,true);

 	//g_pMainFrame->ShowPage(1,1,1,1);
