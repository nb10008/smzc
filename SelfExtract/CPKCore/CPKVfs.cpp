...
BOOL	CPKVfs::Create(LPCTSTR pFileName)
{
	...
	lstrcpy(m_szCPKFileName, pFileName);	//保存文件名
...
//-----------------------------------------------------------------------------
//检测该文件名是否表示解包之后要打入cpk包，比如: data.cpk\system\client.exe,
...
BOOL  CPKVfs::GetFileOrDirName( INT nTableIndex, LPCTSTR lpName )
{
	// 创建文件对象
	BOOL bRet = GetFullNameFromCRC( m_pCPKTable[nTableIndex].dwCRC, lpName );
...
DWORD	CPKVfs::Open(INT nTableIndex, DWORD dwLength, BOOL bCpk)
{
	...
	{
		// 为解压申请内存
		fp->lpMem = m_pMemMgr->Alloc(fp->dwFileSize);
		if( fp->lpMem == NULL )
		{
...
//*********初始化
//-----------------------------------------------------------------------------
INT CPKVfs::InitFileCPKToCPK(LPCTSTR pFullName, DWORD dwFatherCRC, tagCPKTable *pTable)
{
	ASSERT(pFullName);

	if( _stricmp(pFullName, "system\\avatargirl\\girl.fskel") == 0 )
	{
		ASSERT( pFullName );
	}
