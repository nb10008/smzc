...
CPK::CPK()
{
	...
	m_pPackBuffer = CMemMgr::Alloc(GT_CPK_PACK_BUFFER_SIZE);
	m_dwRawBufferSize = GT_CPK_RAW_BUFFER_SIZE;
	m_dwPackBufferSize = GT_CPK_PACK_BUFFER_SIZE;
	...
	if( bDir )
	{
		...
		while( CDiscIOMgr::FindNextFile(dwHandle, &FindData) )
		{
			if( FindData.cFileName[0] != _T('.') )
			{
				...
				this->AddFileOrDir(szTemp, dwCRC, FindData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY);
			}
		}
						   ...
						   DWORD dwSrcFileSize)
{
	...
	if( m_pCPKTable[nTableIndex].dwPackedSize > 0 )
	{
		...
		{
			m_pMD5->MD5ForBuffer((BYTE*)pSrc, dwSrcFileSize, ExtraInfo.byMD5);
			CDiscIOMgr::Write(m_dwCPKHandle, pSrc, dwSrcFileSize);			
		}
		else
		{
			dwBytesToWrite = m_pCPKTable[nTableIndex].dwPackedSize;
			m_pMD5->MD5ForBuffer((BYTE*)m_pPackBuffer, dwBytesToWrite, ExtraInfo.byMD5);
			CDiscIOMgr::Write(m_dwCPKHandle, m_pPackBuffer, dwBytesToWrite);
		}
	}
	else
	{ //没有文件内容,没关系,我们计算名字的md5
		m_pMD5->MD5ForString((char*)pFileName, ExtraInfo.byMD5);
	}
