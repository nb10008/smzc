...
public:		//*****写入(添加,删除等编辑操作)辅助接口*****//
	...
	//*********初始化
	INT InitFileCPKToCPK(LPCTSTR pFullName, DWORD dwFatherCRC, tagCPKTable *pTable);
	//*********写入数据
	// 往包里的当前目录添加文件,这种方式比较特殊，它本身是从另外一个ｃｐｋ包里转移过来
	//因此我们坚持一个原则:对文件内容不压缩(也许他已经压缩过),扩展信息不变
	//*********写入数据
...
public:		//*****辅助相关接口*****//
	...
	//根据index获得文件信息
	BOOL  GetFileOrDirName( INT nTableIndex, LPCTSTR lpName );
	...
	//得到全路径文件名,通过crc
	BOOL	GetFullNameFromCRC(DWORD dwCRC, LPCTSTR szFullName);
	// 由CRC标识得到简单文件名
	...
	BOOL	IsFileForCPK(LPCTSTR szFullName, LPCTSTR szCPKName, LPCTSTR szFileName);
...
protected:
	TCHAR		m_szCPKFileName[MAX_PATH];		//该cpk文件名
