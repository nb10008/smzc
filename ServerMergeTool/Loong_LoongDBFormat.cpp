...
VOID CLoongLoongDBCtrl::FormatQueryEquipBaiBao(Beton::MyStream *pStream,EDBType eDBType)
{
	pStream->SetSelect("equip_baibao",
		...
		// F-Plan 1.6.0 
		"byStarEnhanceLvl,"  "byHolyHoleNum"
		);
...
VOID CLoongLoongDBCtrl::FormatQueryPetData(Beton::MyStream *pStream,EDBType eDBType)
{
	...
	pStream->SetSelect("pet_data",
		...
		//	12			13			14
		"wuxing_energy,	pet_state,	pet_lock,	RemoveFlag,"
...
VOID CLoongLoongDBCtrl::FormatQueryClergyRoleID(Beton::MyStream *pStream,EDBType eDBType)
{
	...
	pStream->FillString(" select RoleID from Clergy_Apollo WHERE RecvOrNot = 1");
	pStream->FillString(" UNION ");
	pStream->FillString(" select RoleID from Clergy_Rabbi where RecvOrNot = 1");
	pStream->FillString(" UNION ");
	pStream->FillString(" select RoleID from Clergy_Peace WHERE RecvOrNot = 1");
...
VOID CLoongLoongDBCtrl::FormatQueryUseKeyCode(Beton::MyStream *pStream,EDBType eDBType)
{
	pStream->SetSelect("UseKeyCode_roleID", "RoleID");
}
...
VOID CLoongLoongDBCtrl::FormatPagingQueryFabao(Beton::MyStream *pStream,  INT64 n64Limit, EDBType eDBType)
{
	pStream->SetSelect("fabao",		"SerialNum,"		"NativeIntelligence,"	"Stage,"				"PracticeValue,"	"AttTypes,"
									...
									//F-Plan 1.5.8
									"ElePierce,"		"EleInjPromoteTimes,"	"EleResPromoteTimes"
									);
...
VOID CLoongLoongDBCtrl::FormatPagingQueryHoly(Beton::MyStream *pStream,INT64 n64Limit,EDBType eDBType)
{
	pStream->SetSelect( "holyman",	"SerialNum,			DevourNum,					EquipmentNumber,		CoValue,					ToDayDevourNum,		EquipSerialIDs,"
									"CostHoly,			HolyDmg,				HolyDef,					Crit,				HolyCritRate,"		
									"ExDamage,					AttackTec,				NeglectToughness,			HolyValue,		MaxDevourNum");

	pStream->FillString(" limit ")<<n64Limit;
	pStream->FillString(",")<<DB_PAGING;
}

VOID CLoongLoongDBCtrl::FormatPagingQueryHolyEquip(Beton::MyStream *pStream,INT64 n64Limit,EDBType eDBType)
{
	pStream->SetSelect( "holyequip",	"SerialNum,			CostHoly,		HolyDmgChg,				HolyDefChg,					CritChg,				HolyCritRateChg,"
...
VOID CLoongLoongDBCtrl::FormatPagingQueryRareArea(Beton::MyStream *pStream,INT64 n64Limit, EDBType eDBType)
{
	...
	//pStream->SetWhere();
	pStream->Clear();
	pStream->FillString(" select * from rarearea where ItemID in (4650001,4650101,4650201,4650301)");
...
VOID CLoongLoongDBCtrl::FormatPagingQueryShortCutBar(Beton::MyStream *pStream,INT64 n64Limit,EDBType eDBType)
{
	pStream->SetSelect("shortcut_quickbar",	"RoleID,"	"quickbardata");
	pStream->FillString(" limit ")<<n64Limit;
	pStream->FillString(",")<<DB_PAGINGSPEC;//分页
}
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
...
BOOL CLoongLoongDBCtrl::FormatInsertEquip(Beton::MyStream *pStream,LPVOID &pData,LPCSTR pszTable,EDBType eDBType)
{
	...
	for(;it != p->end(); ++it)
	{
		...
		// F-Plan 1.6.0 
		pStream->FillString(",byStarEnhanceLvl=")<< (*it)->stOthers.equipSpec.byStarEnhanceLv;
		pStream->FillString(",byHolyHoleNum=")<< (*it)->stOthers.equipSpec.byHolyHoleNum;
...
BOOL CLoongLoongDBCtrl::FormatInsertGuild(Beton::MyStream *pStream,LPVOID &pData,EDBType eDBType)
{
	...
	for(;it != p->end(); ++it)
	{
		...
		pStream->FillString(",GuildValue1=") << (*it)->stOthers.dwGuildValue1;
		pStream->FillString(",GuildValue2=") << (*it)->stOthers.dwGuildValue2;
...
// 		pStream->FillString(",TalentVal4=") <<pRoleDataSave->talent[3].nPoint;
		...
		pStream->FillString(",holy_value=") << pRoleDataSave->nHolyValue;
...
BOOL CLoongLoongDBCtrl::FormatInsertFabao(Beton::MyStream *pStream,LPVOID &pData,EDBType eDBType)
{
	...
	{	
		...
		//F-Plan 1.5.8
		pStream->FillString(",ElePierce=") << (*it)->stOthers.nElePierce;
		pStream->FillString(",EleInjPromoteTimes=") << (*it)->stOthers.wEleInjPromoteTimes;
		pStream->FillString(",EleResPromoteTimes=") << (*it)->stOthers.wEleResPromoteTimes;
		...
		pStream->FillString(",DamageUpgradeTimes=") << (*it)->stOthers.wDamageUpgradeTimes;
		pStream->FillString(",ResistanceUpgradeTimes=") << (*it)->stOthers.wResistanceUpgradeTimes;
...
// F-Plan 1.6.0 
BOOL CLoongLoongDBCtrl::FormatInsertHoly(Beton::MyStream *pStream,LPVOID &pData,EDBType eDBType)
{
	MTRANS_POINTER(p, pData, std::vector <tagTWrap_Loong_Holy*>);
	std::vector <tagTWrap_Loong_Holy*>::iterator it = p->begin();

	Beton::Connection* pCon = m_LoongDB[eDBType].GetFreeConnection();
	for(;it != p->end(); ++it)
	{
		pStream->SetInsert("holyman");

		pStream->FillString  ("SerialNum=") << (*it)->n64Serial;
		pStream->FillString  (" ,DevourNum=") << (*it)->stOthers.nDevourNum;
		pStream->FillString  (" ,EquipmentNumber=") << (*it)->stOthers.n16EquipmentNumber;
		pStream->FillString  (" ,ToDayDevourNum=") << (*it)->stOthers.nToDayEatNum;
		pStream->FillString  (" , EquipSerialIDs='") ;
		pStream->FillBlob((LPVOID)(*it)->stOthers.n64EquipSerialIDs, sizeof((*it)->stOthers.n64EquipSerialIDs),pCon);
		pStream->FillString  ("',CoValue=") << (*it)->stOthers.nCoValue;
		pStream->FillString  (" ,CostHoly=") << (*it)->stOthers.nCostHoly;
		pStream->FillString  (" ,HolyDmg=") << (*it)->stOthers.dwLevelUpAtt[ESAT_Demage];
		pStream->FillString  (" ,HolyDef=") << (*it)->stOthers.dwLevelUpAtt[ESAT_HolyDef];
		pStream->FillString  (" ,Crit=") << (*it)->stOthers.dwLevelUpAtt[ESAT_Crit];
		pStream->FillString  (" ,HolyCritRate=") << (*it)->stOthers.dwLevelUpAtt[ESAT_CritRate];
		pStream->FillString  (" ,ExDamage=") << (*it)->stOthers.dwLevelUpAtt[ESAT_ExDmg];
		pStream->FillString  (" ,AttackTec=") << (*it)->stOthers.dwLevelUpAtt[ESAT_AttackTec];
		pStream->FillString  (" ,NeglectToughness=") << (*it)->stOthers.dwLevelUpAtt[ESAT_NeglectToughness];
		pStream->FillString  (" ,HolyValue=") << (*it)->stOthers.dwLevelUpAtt[ESAT_LingNeng];
		pStream->FillString  (" ,MaxDevourNum=") << (*it)->stOthers.nMaxDevourNum;
...
BOOL CLoongLoongDBCtrl::FormatInsertHolyEquip(Beton::MyStream *pStream,LPVOID &pData,EDBType eDBType)
{
	MTRANS_POINTER(p, pData, std::vector <tagTWrap_Loong_HolyEquip*>);
	std::vector <tagTWrap_Loong_HolyEquip*>::iterator it = p->begin();

	Beton::Connection* pCon = m_LoongDB[eDBType].GetFreeConnection();
	for(;it != p->end(); ++it)
	{
		pStream->SetInsert("holyequip");

		pStream->FillString  ("SerialNum=") << (*it)->n64Serial;
		pStream->FillString  (" ,CostHoly=") << (*it)->stOthers.nCostHoly;
		pStream->FillString  (" ,EnhanceCount=") << (*it)->stOthers.n16EnhanceCount;	
		pStream->FillString  (" ,HolyDmgChg=") << (*it)->stOthers.nHolyAttEffect[ESAT_Demage];
		pStream->FillString  (" ,HolyDefChg=") << (*it)->stOthers.nHolyAttEffect[ESAT_HolyDef];
		pStream->FillString  (" ,CritChg=") << (*it)->stOthers.nHolyAttEffect[ESAT_Crit];
		pStream->FillString  (" ,HolyCritRateChg=") << (*it)->stOthers.nHolyAttEffect[ESAT_CritRate];
		pStream->FillString  (" ,ExDamageChg=") << (*it)->stOthers.nHolyAttEffect[ESAT_ExDmg];
		pStream->FillString  (" ,AttackTecChg=") << (*it)->stOthers.nHolyAttEffect[ESAT_AttackTec];
		pStream->FillString  (" ,NeglectToughnessChg=") << (*it)->stOthers.nHolyAttEffect[ESAT_NeglectToughness];	
...
BOOL CLoongLoongDBCtrl::FormatInsertFabaoNirVanaData(Beton::MyStream *pStream,LPVOID &pData,EDBType eDBType)
{
	...
	for(;it != p->end(); ++it)
	{
		...
		//F-Plan 1.5.8
		pStream->FillString(",ElePierce=") << (*it)->data.nElePierce;
		pStream->FillString(",EleInjPromoteTimes=") << (*it)->data.wEleInjPromoteTimes;
		pStream->FillString(",EleResPromoteTimes=") << (*it)->data.wEleResPromoteTimes;
		...
		pStream->FillString(",DamageUpgradeTimes = ") << (*it)->data.wDamageUpgradeTimes;
		pStream->FillString(",ResistanceUpgradeTimes = ") << (*it)->data.wResistanceUpgradeTimes;
...
// F-Plan 1.5.12 UseKeyCode_roleID
BOOL CLoongLoongDBCtrl::FormatInsertUseKeyCode(Beton::MyStream *pStream,LPVOID &pData,EDBType eDBType)
{
	MTRANS_POINTER(p, pData, std::vector <tagTWrap_Loong_UseKeyCode*>);
	std::vector <tagTWrap_Loong_UseKeyCode*>::iterator it = p->begin();
