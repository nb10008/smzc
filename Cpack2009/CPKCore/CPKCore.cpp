...
CPK::CPK()
{
	...
	m_pMemPool = new MemPool;

	m_pRawBuffer = m_pMemPool->Alloc(GT_CPK_RAW_BUFFER_SIZE);
	
	m_dwPackBufferSize = COMPRESS_SIZE + 2 * 1024 * 1024;
	m_pPackBuffer = m_pMemPool->Alloc( m_dwPackBufferSize  );
	...
	m_pTempArray = new INT[GT_CPK_DEFAULT_FILE];
	m_nArrayNum = GT_CPK_DEFAULT_FILE;
}
//-----------------------------------------------------------------------------
CPK::~CPK() 
{
	m_pMemPool->Free(m_pRawBuffer);
	m_pMemPool->Free(m_pPackBuffer);
	m_dwRawBufferSize = 0;
...
BOOL CPK::IncreaseTable()
{
	...
	else
	{
		...
		// 记录碎片所属的Table项
		memset(p1, -1, sizeof(INT)*dwTable);	
		// 记录Table所属的碎片项
		memset(p2, -1, sizeof(INT)*dwTable);
...
//-----------------------------------------------------------------------------
DWORD CPK::AddDirCyc(LPCTSTR pFullName, DWORD dwFatherCRC)
{
	ASSERT(pFullName);
...
DWORD CPK::CreateDir(LPCTSTR pSimpleDirName, DWORD dwFatherCRC)
{
	...
	DWORD dwCRC = this->Crc(szFullDirName);

	if( this->HaveRepeatCRC(dwCRC) == TRUE )	// 是否有重复CRC
	{
...
extern HWND g_hWndFileName;	// 显示当前处理的文件名的控件
extern HWND g_hWndProgress; // 显示当前进度的控件
extern HWND g_hWndInfo; // 显示当前进度的控件
//-----------------------------------------------------------------------------
// 往包里的当前目录添加文件
//-----------------------------------------------------------------------------
BOOL CPK::AddFile(LPCTSTR pFullName, DWORD dwFatherCRC)
{
	...
	// 需要显示当前的信息以及进度条
	if( g_hWndFileName != NULL && g_hWndProgress != NULL && g_hWndInfo != NULL)
	{
		m_dwFilesToProceed = 1;

		// 对需要处理的文件进行统计
		if( bDir )
		{
			// 用来显示文件处理进度
			// 得到需要处理的文件数以及需要处理的字节数
			m_dwFilesToProceed = CDiscIOMgr::GetFileNumInDir(pFullName);
...
BOOL CPK::ExtractFile(LPCTSTR pDestDir, LPCTSTR pFullName)
{
	...
	if( g_hWndFileName != NULL && g_hWndProgress != NULL && g_hWndInfo != NULL)
	{
		m_dwFilesToProceed = 0;		// 需要处理的文件数
		m_dwFilesCurProceed = 0;	// 已经处理的文件数
...
{
	if( m_nArrayNum < (INT)(m_CPKHeader.dwValidTableNum ))
	{
		FREEARR(m_pTempArray);

		m_pTempArray = new INT[m_CPKHeader.dwValidTableNum];
		m_nArrayNum = m_CPKHeader.dwValidTableNum;
	}
...
BOOL CPK::GetSimpleFileNameFromCRC(LPTSTR pSimpleFileName, DWORD dwCRC)
{
	...
	if (dwCRC == 0)
	{
		Msg("CRC == 0 in function GetFileNameFromCRC");
		return FALSE;
	}
	...
	for( INT n=0; (DWORD)n<m_CPKHeader.dwValidTableNum; n++ )
	{
		if( dwCRC < m_pCPKTable[n].dwCRC )
		{
			...
			for( INT m=m_CPKHeader.dwValidTableNum; m>n; m-- )
			{
				...
				{
					//ASSERT( m_pFragment[0] == 0 ); 
					//ASSERT( m_pIndexToFragment[m-1] != -1 );
...
BOOL CPK::AddSingleFile(LPCTSTR pFullName, DWORD dwFatherCRC)
{
	...
	// 还要处理有重复文件的情况
	DWORD dwCRC = this->Crc(szFullName);
	if( this->HaveRepeatCRC(dwCRC))
	{
		INT nRet = CUtil::MsgBox( MB_OKCANCEL, _T("文件已经存在:%s, 是否覆盖？"), szFullName);
		if( nRet != IDOK )
			return FALSE;

		RemoveFile( szFullName );
	}
	...
	pSrc = m_pMemPool->Alloc( dwSrcFileSize );
...
BOOL CPK::AddFileOrDir(LPCTSTR pFullName, DWORD dwFatherCRC, BOOL bDir)
{
	...
	if( bDir )
	{
		...
		while( CDiscIOMgr::FindNextFile(dwHandle, &FindData) )
		{
			if( FindData.cFileName[0] != _T('.') )
			{
				...
				BOOL bRet = this->AddFileOrDir(szTemp, dwCRC, FindData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY);
	...
	else
	{
		...
		// 显示文件名
		if( g_hWndFileName )
			::SendMessage(g_hWndFileName, WM_SETTEXT, 0, ((LONG)pFullName));

		// 显示进度
		if( g_hWndProgress )
		{
			DWORD dwProceed = (DWORD)((float)m_dwFilesCurProceed/m_dwFilesToProceed*1000.0f);
			SendMessage(g_hWndProgress, PBM_SETPOS, (WPARAM)dwProceed, 0); 
		}

		if( g_hWndInfo )
		{
			sprintf(szTempInfo, "文件%luof%lu  (%d%%)", 
				m_dwFilesCurProceed, m_dwFilesToProceed, 
				(int)((float)m_dwFilesCurProceed/m_dwFilesToProceed*100));
			::SendMessage(g_hWndInfo, WM_SETTEXT, 0, ((LONG)szTempInfo));
		}
...
BOOL CPK::ExtractSingleFile(LPCTSTR pDestDir, DWORD dwCRC)
{
	...
	LPVOID lpInBuffer = m_pMemPool->Alloc(m_pCPKTable[nTableIndex].dwPackedSize);
	LPVOID lpOutBuffer = m_pMemPool->Alloc(m_pCPKTable[nTableIndex].dwOriginSize);
	DWORD dwInBufferSize = m_pCPKTable[nTableIndex].dwPackedSize;
	DWORD dwOutBufferSize = m_pCPKTable[nTableIndex].dwOriginSize;
	...
	if( IS_LARGE_FILE((m_pCPKTable + nTableIndex)->dwFlag) == false )
	{
		...
		//释放读入的部分
		m_pMemPool->Free( lpInBuffer );
	}
	else 
	{
		//释放没有用的lpOutBuffer
		m_pMemPool->Free( lpOutBuffer );
		lpOutBuffer = lpInBuffer;
	}
...
_retry:			
	...
	// 显示文件名
	if( g_hWndFileName )
		::SendMessage(g_hWndFileName, WM_SETTEXT, 0, ((LONG)pDestDir));
	
	// 显示进度
	if( g_hWndProgress )
	{
		DWORD dwProceed = (DWORD)((float)m_dwFilesCurProceed/m_dwFilesToProceed*1000.0f);
		dwProceed = dwProceed > 1000 ? 1000 : dwProceed;
		SendMessage(g_hWndProgress, PBM_SETPOS, (WPARAM)dwProceed, 0); 
	}
...
//得到碎片快数
//-----------------------------------------------------------------------------
DWORD CPK::GetFragmentNum()
{
	return m_CPKHeader.dwFragmentNum;
}
...
//得到所有碎片空间大小
//-----------------------------------------------------------------------------
DWORD CPK::GetFragmentSpace()
{
	DWORD dwSpaceSize = 0;

	for(INT i=0; i<(INT)m_CPKHeader.dwFragmentNum; i++)
	{
		INT nTableIndex = m_pFragment[i];
		ASSERT( IS_FILE_REMOVED(m_pCPKTable[nTableIndex].dwFlag) );	// 确认是碎片


		dwSpaceSize += m_pCPKTable[nTableIndex].dwPackedSize;
