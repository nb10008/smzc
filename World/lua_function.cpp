...
static int SC_AddRoleYuanbao(lua_State* L);
...
// 获取玩家圣灵数量
static int SC_GetRoleHolyNum(lua_State* L);
...
static int SC_GetRoleExp(lua_State* L);
static int SC_SetWarGold(lua_State* L);
static int SC_GetWarGold(lua_State* L);
static int SC_ShowWarResult(lua_State* L);
...
//增加妖精的经验值
static int SC_AddFabaoExp(lua_State* L);
...
//	{"MapPlaySceneMusic",				SC_MapPlaySceneMusic},
	...
	{"ShowWarResult",					SC_ShowWarResult},
...
static const luaL_Reg roleLib[] = 
{
	...
	{"AddRoleYuanbao",					SC_AddRoleYuanbao},
	...
	{"GetRoleHolyNum",				SC_GetRoleHolyNum},
	...
	//获取玩家当前经验。。
	{"GetRoleExp",						SC_GetRoleExp},
	{"SetWarGold",						SC_SetWarGold},
	{"GetWarGold",						SC_GetWarGold},

	//增加妖精的经验值
	{"AddFabaoExp",						SC_AddFabaoExp},
...
// 增加玩家元宝
//-------------------------------------------------------------------------------------
int SC_AddRoleYuanbao(lua_State* L)
{
	DWORD   dwMapID			=	(DWORD)lua_tonumber(L, 1);
	DWORD   dwInstanceID	=	(DWORD)lua_tonumber(L, 2);
	DWORD   dwRoleID		=	(DWORD)lua_tonumber(L, 3);
	...
	pRole->GetCurMgr().IncBagYuanBao(nValue, (ELogCmdID)nCmdID);
...
// 获取玩家圣灵数量
int SC_GetRoleHolyNum(lua_State* L)
{
	DWORD dwRoleID = (DWORD)lua_tonumber(L, 1);
	...
	if( P_VALID(pRole) )
	{	
		INT32 nHolyNum = pRole->GetItemMgr().GetBagSameItemCount2(EIT_Holy);
...
static int SC_GetRoleExp(lua_State* L)
{
	...
	INT32 nHigh = pRole->GetCurLevelExp()/1000000000;
	INT32 nLow = pRole->GetCurLevelExp() % 1000000000;

	lua_pushnumber(L, nHigh);
	lua_pushnumber(L, nLow);
...
static int SC_GetWarGold(lua_State* L)
{
	...
	int nWarGold = pWarRole->GetGold();
	lua_pushnumber(L, nWarGold);
...
static int SC_ShowWarResult(lua_State* L)
{
	DWORD dwMapID			= (DWORD)lua_tonumber(L, 1);
	DWORD dwInstanceID		= (DWORD)lua_tonumber(L, 2);
	BOOL bSuccess			= (DWORD)lua_tonumber(L, 3);

	Map* pMap = g_mapCreator.GetMap(dwMapID, dwInstanceID);
	if(!P_VALID(pMap))	return 0;

	WarRoleMgr* pWarRoleMgr = pMap->GetWarRoleMgr();
	...
	else
	{
		if (TRUE == bSuccess)
		{
			pWarRoleMgr->SetWarResult(EWRR_Win);
		}
		else
		{
			pWarRoleMgr->SetWarResult(EWRR_Lose);
		}
	}
...
static int SC_AddFabaoExp(lua_State* L)
{
	...
	if(P_VALID(pEquip))
	{
		if(MIsFaBao(pEquip))
		{
			...
			INT nHP = pRole->GetAttValue(ERA_HP);
			INT nMP = pRole->GetAttValue(ERA_MP);
			...
			if( change )
			{
				...
				if( P_VALID(pRole->GetScript()) )
					pRole->GetScript()->OnFabaoAttChange(pRole,pFabao,RFAE_LevelUp,pFabao->n16Stage);
			}
