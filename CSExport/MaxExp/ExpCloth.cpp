...
ExpClothMesh::~ExpClothMesh()
{
}

int ExpClothMesh::GetBoneIndex(int boneID, vector<int>& bones)
{
...
int ExpClothMesh::GetVertexIndex(int vertexID, vector<int>& vertices)
{
...
void ExpClothMesh::Build(INode *pNode,TimeValue time,ExpSkeleton *pSkeleton)
{
...
void ExpClothMesh::GenerateSkinVerts(vector<BYTE>& byVBuffer)
{
...
bool ExpClothMesh::GenerateLodMeshs(const vector<BYTE>& byVBuffer)
{
	//LOD层次现在分为三层高、中、低
	//面数分别为原模型的1、1/2、1/8
	for(int i=0;i<GetNumSubMesh();i++)
	{
		...
		LodClothSubMesh *pm=new LodClothSubMesh;
		pm->baseIndex=0;
		...
		for(int j=0;j<numFaces;j++)
		{
			...
			if( !bIsCloth )
			{
				const SkinVert& v0 = m_vertices[p.indices[0]];
				const SkinVert& v1 = m_vertices[p.indices[1]];
				const SkinVert& v2 = m_vertices[p.indices[2]];
				if( v0.boneInfs.size()==0 && v1.boneInfs.size()==0 && v2.boneInfs.size()==0 )
				{
					bIsCloth = true;
				}
			}
		}
		pm->m_isSkin = !bIsCloth;
...
void ExpClothMesh::GenerateBoneOffsetMat(ExpSkeleton *pSkeleton, const FaceBuffer& faces, int lod, 
...
void ExpClothMesh::GenerateBoneOffsetMats(ExpSkeleton *pSkeleton)
{
	//计算每个subMesh的顶点集合
	//计算每个subMesh的骨骼集合
...
void ExpClothMesh::GenerateLodMesh(ID3DXPMesh* pPMesh, FaceBuffer* pFaces, int numFaces)
{
...
SkinVert& ExpClothMesh::GetClothVert(UINT i)
{
	assert(i<m_vertices.size());
	return m_vertices[i];
}

LodSkinSubMesh* ExpClothMesh::GetLodSubMesh(UINT i)
{
	assert(i<m_lodSubMeshs.size());
	return m_lodSubMeshs[i];
}

void ExpClothMesh::BuildAABBox(Vector3& max,Vector3& min)
{
...
//-----------------------------------------
class ClothEnumProc : public ITreeEnumProc  
{
public:
	ClothEnumProc(ExpCloth *pExp)
	{
		m_pExp=pExp;
	}
	~ClothEnumProc()	{}
...
int ExpCloth::BuildFromMaxExportInterface(ExpInterface *pEI,ExpSkeleton *pSkeleton)
{
	m_pSkeleton=pSkeleton;
	//--
	ClothEnumProc enumProc(this);
	pEI->theScene->EnumTree(&enumProc);

	return m_maxNodes.size();
}

const TCHAR* ExpCloth::GetMeshName(UINT i)
{
	return m_maxNodes[i]->GetName();
}

ExpClothMesh *ExpCloth::GetExpClothMesh(int i)
{
...
void ExpCloth::AddMeshNode(INode *pNode)
{
	m_maxNodes.push_back(pNode);

	//--
	TimeValue time=0;
	ExpClothMesh *pExp=new ExpClothMesh(m_pSkeleton,m_pDevice9,m_vertType);
	pExp->Build(pNode,time,m_pSkeleton);

	// 碰撞面
	int faceNum = pExp->GetNumFaces();
	if( faceNum == 2 )
	{
		int numSubMesh = pExp->GetNumSubMesh();
		for( int i=0; i<numSubMesh; i++ )
		{
			ExpSubMesh *pSubMesh = pExp->GetSubMesh(i);
			Cool3D::Polygon& poly = pSubMesh->m_faceArray[0];
			assert( poly.numVert==3 );

			tagBlock block;
			block.v0 = pExp->GetClothVert( poly.indices[0] );
			block.v1 = pExp->GetClothVert( poly.indices[1] );
			block.v2 = pExp->GetClothVert( poly.indices[2] );
