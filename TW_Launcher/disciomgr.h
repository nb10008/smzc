...
public:
	...
	static DWORD Create(LPCSTR lpFileName, DWORD dwDesiredAccess, DWORD dwShareMode, 
	...
	static BOOL  Write(DWORD dwFileHandle, LPCVOID lpBuffer, DWORD dwNumberOfBytesToWrite,
					   LPDWORD lpNumberOfBytesWritten, LPVOID lpOverlapped , DWORD dwStartPos=GT_INVALID);
	static BOOL  Write(DWORD dwFileHandle, LPCVOID lpBuffer, DWORD dwNumberOfBytesToWrite, DWORD dwStartPos=GT_INVALID);
	...
	static BOOL  IsExist(LPCSTR pFileName);
	...
	//提示磁盘空间的状况, 不够就提示
	static BOOL	ClueFreeSpace(char *strDriver, DWORD dwUseSize);

	//得到该盘的剩余可用容量
	static ULARGE_INTEGER	GetDriveFreeSpace(char *strDriver);
