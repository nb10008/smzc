...
//-----------------------------------------------------------------------------
DWORD CDiscIOMgr::Create(LPCSTR lpFileName, DWORD dwDesiredAccess, 
						 ...
						 DWORD dwFlagsAndAttributes, DWORD dwTemplateFile)
{
	...
	{
	case DIOI_Win32:
		return (DWORD)::CreateFileA(lpFileName, dwDesiredAccess, dwShareMode,
					  ...
					  DWORD dwNumberOfBytesToRead)
{
	...
	case DIOI_Win32:
		...
		{
			ErrMsg(_T("CE_CANNOT_READ_DISC_FILE"));
			return FALSE;
		}
...
//-----------------------------------------------------------------------------
BOOL CDiscIOMgr::IsExist(LPCSTR pFileName)
{		
	ASSERT(pFileName);
	...
	case DIOI_Win32:
		{
			...
			HANDLE hHandle = INVALID_HANDLE_VALUE;
			hHandle = ::FindFirstFileA(pFileName, &FindData);
	...
	case DIOI_Win32:
		{
			...
			if (hHandle == INVALID_HANDLE_VALUE)
			{
				ErrMsg(_T("Couldn't open file : %s"), pFileName);
				return FALSE;
			}
			...
			{ 
				ErrMsg(_T("Couldn't get file size : %s"), pFileName);
				CloseHandle(hHandle);
				return FALSE;
			}
			...
			{
				ErrMsg(_T("Couldn't read file : %s"), pFileName);
				CloseHandle(hHandle);
				return FALSE;
			}
...
ULARGE_INTEGER	CDiscIOMgr::GetDriveFreeSpace(char *strDriver)
{
	...
	typedef INT( WINAPI *GETDISKFREESPACE_PROC)(LPCSTR, PULARGE_INTEGER, PULARGE_INTEGER, PULARGE_INTEGER);

	GETDISKFREESPACE_PROC pGetDiskFreeSpaceEx = 
		(GETDISKFREESPACE_PROC)GetProcAddress( GetModuleHandleA("kernel32.dll"), "GetDiskFreeSpaceExA" );
	...
	else 
	{
		...
		BOOL bRet = GetDiskFreeSpaceA(strDriver, 
			&dwSectPerClust, 
			&dwBytesPerSect,
			&dwFreeClusters, 
			&dwTotalClusters);
...
{
	TCHAR buf[500];
	swprintf(buf, _T("%s %u byte, %s"), _T("磁盘空间不足"), dwUseSize, _T("建议先释放一部分空间"));
	...
	{
		INT nRet = MessageBox(NULL, buf, _T("警告"), MB_RETRYCANCEL);
