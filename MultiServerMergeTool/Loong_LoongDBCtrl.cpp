							 ...
							 Fun_pFormat pFunFormat, Fun_pProcRes pFunProcRes)
{
	...
	DWORD dwTime = ( timeGetTime() - dwLastTime )/1000/60;
	TCHAR cTemp[200];
	if ( dwTime > 10 )
	{
		_stprintf(cTemp,_T("执行查询用了 %u\r\n"), dwTime);
...
BOOL CLoongLoongDBCtrl::CreateItem(INT64 n64SerialNum, DWORD ItemTypdID, DWORD AccountID, DWORD RoleID)
{
	...
	//插入物品
	BOOL bRet = Loong_Insert(vecData,EM_DB_SRC_ONE);
	if(bRet == FALSE)
	{
		delete pItem;
		return FALSE;
...
// 领域
BOOL CLoongLoongDBCtrl::Loong_Query(std::vector <tagTWrap_Sheng_Area*> &vecData,EDBType eDBType)
{
	return Load(&vecData,eDBType,&CLoongLoongDBCtrl::FormatQueryArea, &CLoongLoongDBCtrl::ProcResQueryArea);
}
...
// 查询fabao
BOOL CLoongLoongDBCtrl::Loong_Query(std::vector <tagTWrap_Loong_Fabao*>&vecData, EDBType eDBType)
{
	return Load(&vecData,eDBType,&CLoongLoongDBCtrl::FormatQueryFabao,&CLoongLoongDBCtrl::ProcResQueryFabao);
}

// 查询soulcrystal
BOOL CLoongLoongDBCtrl::Loong_Query(std::vector <tagTWrap_Loong_SoulCrystal*>&vecData, EDBType eDBType)
{
	return Load(&vecData,eDBType,&CLoongLoongDBCtrl::FormatQuerySoulCrystal,&CLoongLoongDBCtrl::ProcResQuerySoulCrystal);

}

// 查询FabaoNirVanaData
BOOL CLoongLoongDBCtrl::Loong_Query(std::vector <tagTWrap_Sheng_FabaoNirVanaData*>&vecData, EDBType eDBType)
{
	return Load(&vecData,eDBType,&CLoongLoongDBCtrl::FormatQueryFabaoNirVanaData,&CLoongLoongDBCtrl::ProcResQueryFabaoNirVanaData);
}
...
// 删除源1中的某个角色
BOOL CLoongLoongDBCtrl::DeleteRole( DWORD dwRoleID )
{
	MyStream* pStream = m_LoongDB[EM_DB_SRC_ONE].GetStream();
	if( !P_VALID(pStream) ) 
		return FALSE;
	pStream->Clear();
	pStream->FillString("delete from roledata where RoleID = ")<<dwRoleID;
	BOOL bRes = m_LoongDB[EM_DB_SRC_ONE].Execute(pStream);
	m_LoongDB[EM_DB_SRC_ONE].ReturnStream(pStream);

	return bRes;
}

// 删除源1中某个角色的快捷栏
BOOL CLoongLoongDBCtrl::DeleteRoleQuickBar( DWORD dwRoleID )
{
	MyStream* pStream = m_LoongDB[EM_DB_SRC_ONE].GetStream();
	if( !P_VALID(pStream) ) 
		return FALSE;
	pStream->Clear();
	pStream->FillString("delete from shortcut_quickbar where RoleID = ")<<dwRoleID;
	BOOL bRes = m_LoongDB[EM_DB_SRC_ONE].Execute(pStream);
	m_LoongDB[EM_DB_SRC_ONE].ReturnStream(pStream);
...
// 删除源1中某个角色的物品&装备冷却时间
BOOL CLoongLoongDBCtrl::DeleteRoleItemCDTime( DWORD dwRoleID )
{
	MyStream* pStream = m_LoongDB[EM_DB_SRC_ONE].GetStream();
	if( !P_VALID(pStream) ) 
		return FALSE;
	pStream->Clear();
	pStream->FillString("delete from item_cdtime where RoleID = ")<<dwRoleID;
	BOOL bRes = m_LoongDB[EM_DB_SRC_ONE].Execute(pStream);
	m_LoongDB[EM_DB_SRC_ONE].ReturnStream(pStream);
...
// 删除源1中某个角色的百宝袋记录
BOOL CLoongLoongDBCtrl::DeleteRoleBaiBao( DWORD dwRoleID )
{
	MyStream* pStream = m_LoongDB[EM_DB_SRC_ONE].GetStream();
	if( !P_VALID(pStream) ) 
		return FALSE;
	pStream->Clear();
	pStream->FillString("delete from log_baibao where RoleID = ")<<dwRoleID;
	BOOL bRes = m_LoongDB[EM_DB_SRC_ONE].Execute(pStream);
	m_LoongDB[EM_DB_SRC_ONE].ReturnStream(pStream);
...
// 删除源1中某个角色的skill
BOOL CLoongLoongDBCtrl::DeleteRoleSkill( DWORD dwRoleID )
{
	MyStream* pStream = m_LoongDB[EM_DB_SRC_ONE].GetStream();
	if( !P_VALID(pStream) ) 
		return FALSE;
	pStream->Clear();
	pStream->FillString("delete from skill where RoleID = ")<<dwRoleID;
	BOOL bRes = m_LoongDB[EM_DB_SRC_ONE].Execute(pStream);
	m_LoongDB[EM_DB_SRC_ONE].ReturnStream(pStream);
...
// 删除源1中某个角色的Title
BOOL CLoongLoongDBCtrl::DeleteRoleTitle( DWORD dwRoleID )
{
	MyStream* pStream = m_LoongDB[EM_DB_SRC_ONE].GetStream();
	if( !P_VALID(pStream) ) 
		return FALSE;
	pStream->Clear();
	pStream->FillString("delete from title where RoleID = ")<<dwRoleID;
	BOOL bRes = m_LoongDB[EM_DB_SRC_ONE].Execute(pStream);
	m_LoongDB[EM_DB_SRC_ONE].ReturnStream(pStream);
...
// 删除源1中某个角色的名帖
BOOL CLoongLoongDBCtrl::DeleteRoleVisitingCard( DWORD dwRoleID )
{
	MyStream* pStream = m_LoongDB[EM_DB_SRC_ONE].GetStream();
	if( !P_VALID(pStream) ) 
		return FALSE;
	pStream->Clear();
	pStream->FillString("delete from visiting_card where RoleID = ")<<dwRoleID;
	BOOL bRes = m_LoongDB[EM_DB_SRC_ONE].Execute(pStream);
	m_LoongDB[EM_DB_SRC_ONE].ReturnStream(pStream);
...
// 删除源1中某个账号的道具
BOOL CLoongLoongDBCtrl::DeleteAccountItem( DWORD dwAccountID )
{
	MyStream* pStream = m_LoongDB[EM_DB_SRC_ONE].GetStream();
	if( !P_VALID(pStream) ) 
		return FALSE;
	pStream->Clear();
	pStream->FillString("delete from item where AccountID = ") << dwAccountID;
	BOOL bRes = m_LoongDB[EM_DB_SRC_ONE].Execute(pStream);
	m_LoongDB[EM_DB_SRC_ONE].ReturnStream(pStream);
...
// 删除源1中某个item_baibao
BOOL CLoongLoongDBCtrl::DeleteItemBaiBao( INT64 n64Serial )
{
	MyStream* pStream = m_LoongDB[EM_DB_SRC_ONE].GetStream();
	if( !P_VALID(pStream) ) 
		return FALSE;
	pStream->Clear();
	pStream->FillString("delete from item_baibao where SerialNum = ") << n64Serial;
	BOOL bRes = m_LoongDB[EM_DB_SRC_ONE].Execute(pStream);
	m_LoongDB[EM_DB_SRC_ONE].ReturnStream(pStream);
...
// 删除源1 中属于m_setDirtyItem的equip数据
BOOL CLoongLoongDBCtrl::DeleteEquip( INT64 n64Serial )
{
	MyStream* pStream = m_LoongDB[EM_DB_SRC_ONE].GetStream();
	if( !P_VALID(pStream) ) 
		return FALSE;
	pStream->Clear();
	pStream->FillString("delete from equip where SerialNum = ") << n64Serial;
	BOOL bRes = m_LoongDB[EM_DB_SRC_ONE].Execute(pStream);
	m_LoongDB[EM_DB_SRC_ONE].ReturnStream(pStream);
...
// 删除源1 中属于m_setDirtyItem的equip_baibao数据
BOOL CLoongLoongDBCtrl::DeleteEquipBaiBao( INT64 n64Serial )
{
	MyStream* pStream = m_LoongDB[EM_DB_SRC_ONE].GetStream();
	if( !P_VALID(pStream) ) 
		return FALSE;
	pStream->Clear();
	pStream->FillString("delete from equip_baibao where SerialNum = ") << n64Serial;
	BOOL bRes = m_LoongDB[EM_DB_SRC_ONE].Execute(pStream);
	m_LoongDB[EM_DB_SRC_ONE].ReturnStream(pStream);
...
// 删除源1 中属于m_setDirtyItem的fabao数据
BOOL CLoongLoongDBCtrl::DeleteFabao( INT64 n64Serial )
{
	MyStream* pStream = m_LoongDB[EM_DB_SRC_ONE].GetStream();
	if( !P_VALID(pStream) ) 
		return FALSE;
	pStream->Clear();
	pStream->FillString("delete from fabao where SerialNum = ") << n64Serial;
	BOOL bRes = m_LoongDB[EM_DB_SRC_ONE].Execute(pStream);
	m_LoongDB[EM_DB_SRC_ONE].ReturnStream(pStream);
...
// 删除源1 中属于m_setDirtyItem的SoulCrystal数据
BOOL CLoongLoongDBCtrl::DeleteSoulCrystal( INT64 n64Serial )
{
	MyStream* pStream = m_LoongDB[EM_DB_SRC_ONE].GetStream();
	if( !P_VALID(pStream) ) 
		return FALSE;
	pStream->Clear();
	pStream->FillString("delete from SoulCrystal where SerialNum = ") << n64Serial;
	BOOL bRes = m_LoongDB[EM_DB_SRC_ONE].Execute(pStream);
	m_LoongDB[EM_DB_SRC_ONE].ReturnStream(pStream);
...
// 删除源1中的某个宠物
BOOL CLoongLoongDBCtrl::DeletePet( DWORD dwPetID )
{
	MyStream* pStream = m_LoongDB[EM_DB_SRC_ONE].GetStream();
	if( !P_VALID(pStream) ) 
		return FALSE;
	pStream->Clear();
	pStream->FillString("delete from pet_data where pet_id = ") << dwPetID;
	BOOL bRes = m_LoongDB[EM_DB_SRC_ONE].Execute(pStream);
	m_LoongDB[EM_DB_SRC_ONE].ReturnStream(pStream);
...
// 删除源1中的某条Buff数据
BOOL CLoongLoongDBCtrl::DeleteOneBuff( DWORD dwRoleID, DWORD dwBuffID )
{
	MyStream* pStream = m_LoongDB[EM_DB_SRC_ONE].GetStream();
	if( !P_VALID(pStream) ) 
		return FALSE;
	pStream->Clear();
	pStream->FillString("delete from buff where RoleID = ") << dwRoleID;
...
{
	if( !DeleteRarearea(eDBType)	||
			!sLoongBeton.DaleteArea(eDBType)		||
