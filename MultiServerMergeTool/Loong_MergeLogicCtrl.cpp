											...
											std::vector <tagTWrap_Longin_RoleName*> &vecTar)
{
	...
	// 直接插入
	BOOL bRet = sLoginBeton.LoginDB_Accout_Insert(vecTar,EM_DB_SRC_ONE);
	if(bRet == FALSE)
	{
		return bRet;
	}
	...
	for(;itSrc2!= vecSrc2.end();++itSrc2)
	{
		...
		// 试一次
		sLoginBeton.LoginDB_Accout_Insert(vecTar,EM_DB_SRC_ONE);
											...
											std::vector <tagTWrap_Loong_RoleData*> &vecTar)
{
	...
	for(;itSrc1!= vecSrc1.end();++itSrc1)
	{
		// 源1有源2也有的用户合并角色时，可能会过滤掉源1中一些级别低的角色，故需要对源1清空再插入
		sLoongBeton.DeleteRole((*itSrc1)->dwRoleID);
	...
	for(;itSrc1!= vecSrc1.end();++itSrc1)
	{
		...
		if(emTBType == EM_TB_ITEM_BAIBAO)
		{
			if(IsDirtyRoleData((*itSrc1)->stOthers.dwOwnerID))
			{
				...
				// 删除源1中属于DirtyRole的item_baibao
				sLoongBeton.DeleteItemBaiBao((*itSrc1)->stOthers.n64Serial);
			}		
		}
		else
		{
											...
											std::vector <tagTWrap_Loong_Buff*> &vecTar)
{
	...
	for(;itSrc1!= vecSrc1.end();++itSrc1)
	{
		...
		{
			// 删除源1中DirtyRole的Buff
			sLoongBeton.DeleteOneBuff((*itSrc1)->dwRoleID, (*itSrc1)->stOthers.dwBuffID );
			continue;
		}
											...
											ETBEquipType eTBType)
{
	...
	for(;itSrc1!= vecSrc1.end();++itSrc1)
	{
		...
		{
			if ( eTBType == EM_TB_EQUIP)
			{
				// 删除源1 中属于m_setDirtyItem的equip数据
				sLoongBeton.DeleteEquip( (*itSrc1)->n64SerialNum );
			}
			else
			{
				// 删除源1 中属于m_setDirtyItem的equip_baibao数据
				sLoongBeton.DeleteEquipBaiBao((*itSrc1)->n64SerialNum );
			}
											 ...
											 std::vector <tagTWrap_Loong_ItemCDTime*> &vecTar)
{
	...
	for(;itSrc1!= vecSrc1.end();++itSrc1)
	{
		if(IsDirtyRoleData((*itSrc1)->dwRoleID))
		{
			// 删除源1中DirtyRole的物品&装备冷却时间
			sLoongBeton.DeleteRoleItemCDTime((*itSrc1)->dwRoleID);
											 ...
											 std::vector <tagTWrap_Loong_LogBaiBao*> &vecTar)
{
	...
	for(;itSrc1!= vecSrc1.end();++itSrc1)
	{
		if(IsDirtyRoleData((*itSrc1)->stOthers.dwRoleID))
		{
			// 删除源1中DirtyRole的百宝袋记录
			sLoongBeton.DeleteRoleBaiBao((*itSrc1)->stOthers.dwRoleID);
			continue;
		}			
											 ...
											 std::vector <tagTWrap_Loong_PetData*> &vecTar)
{
	...
	for(;itSrc1!= vecSrc1.end();++itSrc1)
	{
		...
		{
			// 删除源1中DirtyRole的宠物
			sLoongBeton.DeletePet((*itSrc1)->stOthers.dwPetID);
			// continue;
		}
											 ...
											 std::vector <tagTWrap_Loong_Skill*> &vecTar)
{
	...
	for(;itSrc1!= vecSrc1.end();++itSrc1)
	{
		if(IsDirtyRoleData((*itSrc1)->dwRoleID))
		{
			// 删除源1中DirtyRole的Skill
			sLoongBeton.DeleteRoleSkill((*itSrc1)->dwRoleID);
			// continue;
		}			
											 ...
											 std::vector <tagTWrap_Loong_Title*> &vecTar)
{
	...
	for(;itSrc1!= vecSrc1.end();++itSrc1)
	{
		if(IsDirtyRoleData((*itSrc1)->dwRoleID))
		{
			// 删除源1中DirtyRole的Title
			sLoongBeton.DeleteRoleTitle((*itSrc1)->dwRoleID);
			// continue;
		}			
											 ...
											 std::vector <tagTWrap_Loong_VisitingCard*> &vecTar)
{
	...
	for(;itSrc1!= vecSrc1.end();++itSrc1)
	{
		...
		{
			// 删除源1中某个角色的名帖
			sLoongBeton.DeleteRoleVisitingCard((*itSrc1)->stOthers.dwRoleID);
			continue;
		}
											...
											std::vector <tagTWrap_Loong_ShortCutBar*> &vecTar)
{
	...
	for(;itSrc1!= vecSrc1.end();++itSrc1)
	{
		...
		{
			// 删除源1中DirtyRole的快捷栏
			sLoongBeton.DeleteRoleQuickBar((*itSrc1)->dwRoleID);
			// continue;
		}
					  ...
					  std::vector <tagTWrap_Loong_Fabao*> &vecTar)
{
	...
	for(;itSrc1!= vecSrc1.end();++itSrc1)
	{
		...
		{
			// 删除源1 fabao中属于m_setDirtyItem的数据
			sLoongBeton.DeleteFabao((*itSrc1)->n64Serial);
		}
											...
											std::vector <tagTWrap_Loong_SoulCrystal*> &vecTar)
{
	...
	for(;itSrc1!= vecSrc1.end();++itSrc1)
	{
		if(m_setDirtyItem.end() != m_setDirtyItem.find((*itSrc1)->n64SerialNum))
		{
			// 删除源1 SoulCrystal中属于m_setDirtyItem的数据
			sLoongBeton.DeleteSoulCrystal((*itSrc1)->n64SerialNum);
			// continue;
		}	
