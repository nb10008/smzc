...
public:
	...
	// 删除源1中某个角色的快捷栏
	BOOL DeleteRoleQuickBar( DWORD dwRoleID );

	// 删除源1中某个角色的物品&装备冷却时间
	BOOL DeleteRoleItemCDTime( DWORD dwRoleID );

	// 删除源1中某个角色的百宝袋记录
	BOOL DeleteRoleBaiBao( DWORD dwRoleID );

	// 删除源1中某个角色的skill
	BOOL DeleteRoleSkill( DWORD dwRoleID );

	// 删除源1中某个角色的Title
	BOOL DeleteRoleTitle( DWORD dwRoleID );

	// 删除源1中某个角色的名帖
	BOOL DeleteRoleVisitingCard( DWORD dwRoleID );

	// 删除源1中某个账号的道具
	BOOL DeleteAccountItem( DWORD dwAccountID );

	// 删除源1中某个item_baibao
	BOOL DeleteItemBaiBao( INT64 n64Serial );

	// 删除源1 中属于m_setDirtyItem的equip数据
	BOOL DeleteEquip( INT64 n64Serial );

	// 删除源1 中属于m_setDirtyItem的equip_baibao数据
	BOOL DeleteEquipBaiBao( INT64 n64Serial );

	// 删除源1 中属于m_setDirtyItem的fabao数据
	BOOL DeleteFabao( INT64 n64Serial );

	// 删除源1 中属于m_setDirtyItem的SoulCrystal数据
	BOOL DeleteSoulCrystal( INT64 n64Serial );

	// 删除源1中的某个宠物
	BOOL DeletePet( DWORD dwPetID );

	// 删除源1中的某条Buff数据
	BOOL DeleteOneBuff( DWORD dwRoleID, DWORD dwBuffID );
...
//	BOOL Loong_Query(std::vector <tagTWrap_Loong_RoleData*>&,EDBType eDBType);
	...
	// 领域
	BOOL Loong_Query(std::vector <tagTWrap_Sheng_Area*> &vecData,EDBType eDBType);

	// 名帖
	BOOL Loong_Query(std::vector <tagTWrap_Loong_VisitingCard*> &vecData,EDBType eDBType);
	...
	BOOL Loong_Query(std::vector <tagTWrap_Loong_Skill*>&, EDBType eDBType);
	// 查询fabao
	BOOL Loong_Query(std::vector <tagTWrap_Loong_Fabao*>&, EDBType eDBType);
	// 查询soulcrystal
	BOOL Loong_Query(std::vector <tagTWrap_Loong_SoulCrystal*>&, EDBType eDBType);
	// 查询FabaoNirVanaData
	BOOL Loong_Query(std::vector <tagTWrap_Sheng_FabaoNirVanaData*>&, EDBType eDBType);
...
//	VOID FormatQuerySkill(Beton::MyStream *pStream,EDBType eDBType);
	...
	VOID FormatQuerySkill(Beton::MyStream *pStream,EDBType eDBType);
	VOID FormatQueryFabao(Beton::MyStream *pStream,EDBType eDBType);
	VOID FormatQuerySoulCrystal(Beton::MyStream *pStream,EDBType eDBType);
	VOID FormatQueryFabaoNirVanaData(Beton::MyStream *pStream,EDBType eDBType);
	VOID FormatQueryArea(Beton::MyStream *pStream,EDBType eDBType);
