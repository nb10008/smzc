...
,m_pMyFrame(NULL)
{
	...
	sizerTmp->Add(CreateSizerWithTextAndLabel(this, g_StrTable[_T("Year")], wxID_ANY, &m_text_year, false, &validator), 0, wxGROW | wxALL, CONTROL_BORDER);
	sizerTmp->Add(CreateSizerWithTextAndLabel(this, g_StrTable[_T("Month")], wxID_ANY, &m_text_month, false, &validator), 0, wxGROW | wxALL, CONTROL_BORDER);
	sizerTmp->Add(CreateSizerWithTextAndLabel(this, g_StrTable[_T("Date")], wxID_ANY, &m_text_day, false, &validator), 0, wxGROW | wxALL, CONTROL_BORDER);
	sizerTmp->Add(CreateSizerWithTextAndLabel(this, g_StrTable[_T("Hour")], wxID_ANY, &m_text_hour, false, &validator), 0, wxGROW | wxALL, CONTROL_BORDER);
	sizerTmp->Add(CreateSizerWithTextAndLabel(this, g_StrTable[_T("Minute")], wxID_ANY, &m_text_min, false, &validator), 0, wxGROW | wxALL, CONTROL_BORDER);
...
// 				if (time == 0)
// 				{
// 					_sntprintf(szTmpLog, X_HUGE_STRING, _T("\n【右下角公告】| 服务器：所有 | 执行人：%s | 标题：%s | 内容：%s | 发送时间：即时 |\n"), 
// 						g_Tester.c_str(), strTitle.c_str(), strContent.c_str());
// 				}
// 				else
// 				{
// 					_sntprintf(szTmpLog, X_HUGE_STRING, _T("\n【右下角公告】| 服务器：所有 | 执行人：%s | 标题：%s | 内容：%s | 发送时间：20%d %d-%d %d:%d:%d |\n"), 
// 						g_Tester.c_str(), strTitle.c_str(), strContent.c_str(), time.year, time.month, time.day, time.hour, time.min, time.sec);
// 				}
				
				//g_RNLog.Write(szTmpLog); 
			}
			else
			{
				TCHAR byInfo[2048] = {0};
...
// 					if(time == 0)
// 					{
// 						_sntprintf(szTmpLog, X_HUGE_STRING, _T("\n【右下角公告】| 服务器：%s | 执行人：%s | 标题：%s | 内容：%s | 发送时间：即时 |\n"), 
// 							szWorldName, g_Tester.c_str(), strTitle.c_str(), strContent.c_str());
// 					}
// 					else
// 					{
// 						_sntprintf(szTmpLog, X_HUGE_STRING, _T("\n【右下角公告】| 服务器：%s | 执行人：%s | 标题：%s | 内容：%s | 发送时间：20%d %d-%d %d:%d:%d |\n"), 
// 							szWorldName, g_Tester.c_str(), strTitle.c_str(), strContent.c_str(), time.year, time.month, time.day, time.hour, time.min, time.sec);
// 					}
