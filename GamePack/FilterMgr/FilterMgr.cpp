...
VOID CFilterMgr::Filter(tagFilterParam *pParam)
{
	...
	case FMF_MiniLzo1040:
		if (pParam->bCompress)	// compress
		{
			...
			else
			{
				...
				Msg(_T("MiniLzo compression failed: %d\n"), nResult);
			}
		}
		else	// decompress
		{
			...
			else
			{
				...
				pParam->bSuccess = FALSE;
				Msg(_T("MiniLzo decompression failed: %d\n", nResult));
			}
		}
		break;
	...
	case FMF_Lzss:
		if (pParam->bCompress)	// compress
		{
			nResult = Lzss_Compress((BYTE*)pParam->lpMemIn,
				pParam->dwInDataSize,
				(BYTE*)pParam->lpMemOut,
				pParam->dwOutDataSize);
			if( nResult == TRUE )
			{
				pParam->bSuccess = TRUE;
				//Msg("Compressed %lu bytes into %lu bytes\n",
			...
			{
				pParam->bSuccess = FALSE;
				Msg(_T("lzss compression failed: %d\n"), nResult);
			}
		}
		else	// decompress
		{
			nResult = Lzss_DeCompress((BYTE*)pParam->lpMemIn,
				pParam->dwInDataSize,
				(BYTE*)pParam->lpMemOut,
				pParam->dwOutDataSize);
			if(nResult == TRUE )
			...
			else
			{
				// for LZO, this should never happen
				pParam->bSuccess = FALSE;
				Msg( _T("lzss decompression failed: %d\n" ), nResult);
			}
		}
		break;
	default:
