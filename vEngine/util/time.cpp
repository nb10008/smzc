...
{
	//if( !destTime.IsValid() || !srcTime.IsValid() )
	//	return 0;
...
INT CompareDwordTime( tagDWORDTime& dwTargetTime, tagDWORDTime& dwSrcTime )
{
	//if(!dwTargetTime.IsValid() && !dwSrcTime.IsValid())
	//	return (DWORD)dwTargetTime < (DWORD)dwSrcTime;
	//else if( !dwTargetTime.IsValid() )
	//	return -1;
	//else if( !dwSrcTime.IsValid() )
	//	return 1;

	if( dwTargetTime.year < dwSrcTime.year )
		return -1;

	if( dwTargetTime.year > dwSrcTime.year )
		return 1;

	if( dwTargetTime.month < dwSrcTime.month )
		return -1;

	if( dwTargetTime.month > dwSrcTime.month )
		return 1;

	if( dwTargetTime.day < dwSrcTime.day )
		return -1;

	if( dwTargetTime.day > dwSrcTime.day )
		return 1;

	if( dwTargetTime.hour < dwSrcTime.hour )
		return -1;

	if( dwTargetTime.hour > dwSrcTime.hour )
		return 1;

	if( dwTargetTime.min < dwSrcTime.min )
		return -1;
	
	if( dwTargetTime.min > dwSrcTime.min )
		return 1;

	if( dwTargetTime.sec < dwSrcTime.sec )
		return -1;
...
//BOOL operator == (const tagDWORDTime & left,const tagDWORDTime & right)
//{
...
//	if( !(left == right) )
//	{
//		return CalcTimeDiff(left,right) == 0;
//	}
//	return FALSE;
//}
//BOOL operator <= (const tagDWORDTime & left,const tagDWORDTime & right)
...
//	return left == right || left > right;
//}
tagDWORDTime& operator += (tagDWORDTime & left, INT second)
{
	left = IncreaseTime(left,second);
	return left;
}
tagDWORDTime& operator -= (tagDWORDTime &left,INT second)
{
	left = DecreaseTime(left,second);
	return left;
}
