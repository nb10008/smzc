...
CPropertyList::CPropertyList():m_szFileFilter("All Files|*.*||")
{
}
...
COLORREF CPropertyList::Str2Color(const char* szRGBStr)
{
	COLORREF initClr;
...
void CPropertyList::UpdateVal(const char* szName,Color4f& v,int index,bool bSave)
{
	...
	{
		szTmp.Format("RGB(%d,%d,%d)",
			(BYTE)(v.R*255),(BYTE)(v.G*255),(BYTE)(v.B*255));
		pItem=new CPropertyItem(szName,szTmp,PIT_COLOR,"");
		AddPropItem(pItem);
	}
}

void CPropertyList::UpdateVal(const char* szName,bool& v,int index,bool bSave)
{
	CPropertyItem *pItem=NULL;
	CString szTmp;
	...
	{
		pItem=(CPropertyItem *)GetItemDataPtr(index);
		v=(pItem->m_curValue=="true");
	}
	else
	{
		szTmp=v?"true":"false";
		pItem=new CPropertyItem(szName,szTmp,PIT_COMBO,"true|false|");
		AddPropItem(pItem);
	}
}

void CPropertyList::UpdateVal(const char* szName,char* szBuf,int bufSize,int index,bool bSave,bool bFileName)
{
	CPropertyItem *pItem=NULL;
	CString szTmp;
	...
	if(bSave)
	{
		...
		strncpy(szBuf,pItem->m_curValue,bufSize-1);
	}
	else
	{
		szTmp=szBuf;
		...
		if(bFileName)
			type=PIT_FILE;
		pItem=new CPropertyItem(szName,szTmp,type,"");
		AddPropItem(pItem);
	}
}

void CPropertyList::UpdateVal(const char* szName,float& v,int index,bool bSave)
{
	CPropertyItem *pItem=NULL;
	CString szTmp;
	if(bSave)
	{
		pItem=(CPropertyItem *)GetItemDataPtr(index);
		v=atof(pItem->m_curValue);
	}
	else
	{
		szTmp.Format("%.2f",v);
		pItem=new CPropertyItem(szName,szTmp,PIT_EDIT,"");
		AddPropItem(pItem);
	}
}

void CPropertyList::UpdateVal(const char* szName,int& v,int index,bool bSave)
{
	CPropertyItem *pItem=NULL;
	CString szTmp;
	if(bSave)
...
void CPropertyList::UpdateVal(const char* szName,BYTE& v,int index,bool bSave)
{
	CPropertyItem *pItem=NULL;
	CString szTmp;
	if(bSave)
