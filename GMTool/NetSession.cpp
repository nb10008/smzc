...
void NetSession::RegiserHandleMessage()
{
	...
	m_net_command_mgr.Register("NGMTS_AddFilterWords",		&NetSession::OnNGMTS_AddFilterWords);
	m_net_command_mgr.Register("NGMTS_DelFilterWords",		&NetSession::OnNGMTS_DelFilterWords);
	m_net_command_mgr.Register("NGMTS_GuildGodMiracle",		&NetSession::OnNGMTS_GuildGodMiracle);
	m_net_command_mgr.Register("NGMTS_ClearSafeCode",		&NetSession::OnNGMTS_ClearSafeCode);
	m_net_command_mgr.Register("NGMTS_CloseGuard",			&NetSession::OnNGMTS_CloseGuard);
}
...
DWORD NetSession::OnNGMTS_AddFilterWords( tagNetCmd* pMsg )
{
	tagNGMTS_AddFilterWords* pRecv = (tagNGMTS_AddFilterWords*)pMsg;

	tagAddFilterWords evt(_T("tagAddFilterWords"));
	evt.dwErrorCode = pRecv->dwErrorCode;
	sUserFrameMgrPgr->SendEvent(&evt);
	return 0;
}

DWORD NetSession::OnNGMTS_DelFilterWords( tagNetCmd* pMsg )
{
	tagNGMTS_DelFilterWords* pRecv = (tagNGMTS_DelFilterWords*)pMsg;

	tagDelFilterWords evt(_T("tagDelFilterWords"));
	evt.dwErrorCode = pRecv->dwErrorCode;
	sUserFrameMgrPgr->SendEvent(&evt);
	return 0;
}

DWORD NetSession::OnNGMTS_GuildGodMiracle(tagNetCmd* pMsg)
{
	tagNGMTS_GuildGodMiracle* pRecv = (tagNGMTS_GuildGodMiracle*)pMsg;
	if (!P_VALID(pRecv))
	{
		return GT_INVALID;
	}

	tagGuildGodMiracle evt(_T("tagGuildGodMiracle"));
	evt.dwErrorCode = pRecv->dwErrorCode;
	sUserFrameMgrPgr->SendEvent(&evt);
...
DWORD NetSession::OnNGMTS_ClearSafeCode( tagNetCmd* pMsg )
{
	tagNGMTS_ClearSafeCode* pRecv = (tagNGMTS_ClearSafeCode*)pMsg;
	if (!P_VALID(pRecv))
	{
		return GT_INVALID;
	}

	tagClearSafeCode evt(_T("tagClearSafeCode"));
	evt.dwErrorCode = pRecv->dwErrorCode;
	sUserFrameMgrPgr->SendEvent(&evt);
	return 0;
}

DWORD NetSession::OnNGMTS_CloseGuard( tagNetCmd* pMsg )
{
	tagNGMTS_CloseGuard* pRecv = (tagNGMTS_CloseGuard*)pMsg;
	if (!P_VALID(pRecv))
	{
		return GT_INVALID;
	}

	tagCloseGuard evt(_T("tagCloseGuard"));
	evt.dwErrorCode = pRecv->dwErrorCode;
	sUserFrameMgrPgr->SendEvent(&evt);
	return 0;
}