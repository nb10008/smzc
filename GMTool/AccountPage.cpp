...
BEGIN_EVENT_TABLE(AccountPage, wxPanel)
	...
	EVT_BUTTON(AccountPage_ClearSafeCode,AccountPage::OnClearSafeCode)
	EVT_BUTTON(AccountPage_CloseGuard,AccountPage::OnCloseGuard)
END_EVENT_TABLE()
																						  ...
																						  wxNO_FULL_REPAINT_ON_RESIZE | wxCLIP_CHILDREN | wxTAB_TRAVERSAL)
{
	...
	//清空角色安全码
	wxStaticBox *boxSafeCode = new wxStaticBox(this, wxID_ANY, g_StrTable[_T("SafeCode")]);
	wxSizer* sizerBoxSafeCode = new wxStaticBoxSizer(boxSafeCode, wxHORIZONTAL);	

	wxControl *controlSCAccountName = new wxStaticText(this, wxID_ANY, g_StrTable[_T("Account")]);	
	sizerBoxSafeCode->Add(controlSCAccountName, 0, wxALL, CONTROL_BORDER);
	m_text_SCAccountName = new wxTextCtrl(this, wxID_ANY, wxT(""), wxDefaultPosition, wxDefaultSize);
	sizerBoxSafeCode->Add(m_text_SCAccountName, 0, wxALL, CONTROL_BORDER);
	m_btn_ClearSafeCode = new wxButton(this, AccountPage_ClearSafeCode, g_StrTable[_T("ClearSafeCode")]);
	sizerBoxSafeCode->Add(m_btn_ClearSafeCode, 0, wxALL, CONTROL_BORDER);

	//关闭防沉迷
	wxStaticBox *boxCloseGuard = new wxStaticBox(this, wxID_ANY, g_StrTable[_T("Guard")]);
	wxSizer* sizerBoxCloseGuard = new wxStaticBoxSizer(boxCloseGuard, wxHORIZONTAL);	

	wxControl *controlCGAccountName = new wxStaticText(this, wxID_ANY, g_StrTable[_T("Account")]);	
	sizerBoxCloseGuard->Add(controlCGAccountName, 0, wxALL, CONTROL_BORDER);
	m_text_CGAccountName = new wxTextCtrl(this, wxID_ANY, wxT(""), wxDefaultPosition, wxDefaultSize);
	sizerBoxCloseGuard->Add(m_text_CGAccountName, 0, wxALL, CONTROL_BORDER);
	m_btn_CloseGuard = new wxButton(this, AccountPage_CloseGuard, g_StrTable[_T("CloseGuard")]);
	sizerBoxCloseGuard->Add(m_btn_CloseGuard, 0, wxALL, CONTROL_BORDER);
	...
	sizerAll->Add(sizerBoxSafeCode,	0, wxGROW|wxALL, CONTROL_BORDER);
	sizerAll->Add(sizerBoxCloseGuard,	0, wxGROW|wxALL, CONTROL_BORDER);
	...
	sUserFrameMgrPgr->RegisterEventHandle(_T("tagClearSafeCode"),			(FRAMEEVENTPROC)m_Trunk.sfp1(&UserFrame::OnClearSafeCodeEvent));
	sUserFrameMgrPgr->RegisterEventHandle(_T("tagCloseGuard"),				(FRAMEEVENTPROC)m_Trunk.sfp1(&UserFrame::OnCloseGuardEvent));
}
...
void AccountPage::OnClearSafeCode(wxCommandEvent& event)
{
	// 检查权限
	if( !sNetSessionMgrPtr->VerifyGMPrivilege(Crc32("NGMTC_ClearSafeCode")) )
	{
		wxMessageBox(g_StrTable[_T("GMPrivilegeForbit")], wxT(""), wxOK|wxCENTRE, this);
		return;
	}
	...
	if( P_VALID(pCheckList) )
	{
		...
		tstring strAccountName = m_text_SCAccountName->GetValue();
		...
		for( ; it!=vec.end(); it++ )
		{
			...
			{
				tagNGMTC_ClearSafeCode msg;
				msg.dwSectionID = id.section_id;
				msg.dwWorldID = id.world_id;				
				_sntprintf(msg.szAccountName, X_SHORT_NAME, strAccountName.c_str());
...
DWORD AccountPage::OnClearSafeCodeEvent(tagUserEvent* pGameEvent)
{
	if( pGameEvent->strEventName == _T("tagClearSafeCode") )
	{
		tagClearSafeCode* pEvt = (tagClearSafeCode*)pGameEvent;
		if (!P_VALID(pEvt))
		{
			return 0;
		}	
		...
		m_text_SCAccountName->Clear();
	}

	return 0;
}

void AccountPage::OnCloseGuard(wxCommandEvent& event)
{
	// 检查权限
	if( !sNetSessionMgrPtr->VerifyGMPrivilege(Crc32("NGMTC_CloseGuard")) )
	{
		wxMessageBox(g_StrTable[_T("GMPrivilegeForbit")], wxT(""), wxOK|wxCENTRE, this);
		return;
	}
	...
	if( P_VALID(pCheckList) )
	{
		...
		tstring strAccountName = m_text_CGAccountName->GetValue();
...
DWORD AccountPage::OnCloseGuardEvent(tagUserEvent* pGameEvent)
{
	if( pGameEvent->strEventName == _T("tagCloseGuard") )
	{
		tagCloseGuard* pEvt = (tagCloseGuard*)pGameEvent;
		if (!P_VALID(pEvt))
		{
			return 0;
		}	
