...
bool MyApp::OnInit()
{
	...
	// 初始化配置文件
	if( !g_StrTable.Init(NULL, _T("gmtool.xml")) )
	{
		IMSG(_T("Init gmtool.xml Fail"));
		return false;
	}
												  ...
												  wxDEFAULT_FRAME_STYLE | wxNO_FULL_REPAINT_ON_RESIZE|wxCLIP_CHILDREN|wxTAB_TRAVERSAL)
{
	...
	SysMgrPage* sysmgr_page = new SysMgrPage(m_book, g_StrTable[_T("SysMgrPage")]);
	if ( P_VALID(sysmgr_page) ) sysmgr_page->SetMyFrame(this);
	...
	m_book->AddPage(sysmgr_page, sysmgr_page->GetLabel(), false);
