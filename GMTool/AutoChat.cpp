...
void AutoChatPage::OnUseFileOperation(wxCommandEvent& event)
{
	...
	if( P_VALID(pCheckList) )
	{
		...
		char* szText = (char*)new BYTE[X_HUGE_STRING];
		TCHAR* szContent = (TCHAR*) new BYTE[X_HUGE_STRING];
		if (!P_VALID(szText) || !P_VALID(szContent))
		{
			return;
		}

		ZeroMemory(szText, X_HUGE_STRING);
		ZeroMemory(szContent, X_HUGE_STRING);
		int nCount = 0;
		BYTE byHour = 0;
		BYTE byMin = 0;
		...
		while (infile.getline(&szText[0], X_HUGE_STRING))
		{
			...
			for( itEx=mapIDEx.begin(); itEx!=mapIDEx.end(); itEx++ )
			{
				...
				{
					TCHAR* byInfo = (TCHAR*) new BYTE[1024];
					if (!P_VALID(byInfo))
					{
						SAFE_DEL(szContent);
						SAFE_DEL(szText);
