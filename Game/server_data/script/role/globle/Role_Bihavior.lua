...
function x_CanUseDakNPC(MapID, InstanceId, RoleID, ToMap, x, y, z)
	if role.IsRoleHaveBuff(MapID, InstanceID, RoleID, 61016) then
		...
		return false
	elseif MapID == 3390517288 or ToMap == 3390517288 then
		return false
	else
...
function x_OnCompleteBoardQuest(MapID, InstanceID, RoleID, QuestID, BoardID, Money, Exp)
	...
	if role.IsRoleHaveBuff(MapID, InstanceID, RoleID, 90081) then
		local Exp1 = 1.5*Exp
		role.AddRoleExp(MapID, InstanceID, RoleID, Exp1)
	end
	if role.IsRoleHaveBuff(MapID, InstanceID, RoleID, 90082) then
		local Exp1 = 2*Exp
		role.AddRoleExp(MapID, InstanceID, RoleID, Exp1)
	end
	if role.IsRoleHaveBuff(MapID, InstanceID, RoleID, 90083) then
		local Exp1 = 2.5*Exp
		role.AddRoleExp(MapID, InstanceID, RoleID, Exp1)
	end
	if role.IsRoleHaveBuff(MapID, InstanceID, RoleID, 90084) then
		local Exp1 = 3*Exp
		role.AddRoleExp(MapID, InstanceID, RoleID, Exp1)
	end
	if role.IsRoleHaveBuff(MapID, InstanceID, RoleID, 90085) then
		local Exp1 = 4*Exp
		role.AddRoleExp(MapID, InstanceID, RoleID, Exp1)
	end
end
