...
function x_OnCutStone(MapID, InstanceID, RoleID, DianJinID, TypeID, NewStoneTypeID)
	...
	if NewStoneTypeID == 3050087 and Item_ID[TypeID] ~= nil then
		...
		if FreeSize >= 3 then
			...
			if Item_ID[TypeID].ID[2] ~= nil and math.random(100) <= Item_ID[TypeID].Percent[2] then
			    role.AddRoleItem(MapID, InstanceID, RoleID, Item_ID[TypeID].ID[2],Item_ID[TypeID].num[2], -1, 8, 420)
			end
			if Item_ID[TypeID].ID[3] ~= nil and math.random(100) <= Item_ID[TypeID].Percent[3] then
...
function x_OnChooseGrowAtt(RoleID,MapID,InstanceID, TypeID, color)
	...
	local function GetEqupPosAndType(TypeID)  --通过表中记录的数据来获取该分配哪个段的属性
		...
		elseif TypeID == 8030012 or TypeID == 8040012 then
		    type2 = 5
		elseif TypeID == 8049003 or TypeID == 8039003 or TypeID == 8029003 or TypeID == 8019003 or TypeID == 8111916 or TypeID == 8111917 or TypeID == 8111918 or TypeID == 8111919 or TypeID == 8110916 or TypeID == 8110917 or TypeID == 8110918 or TypeID == 8110919 or TypeID == 8019006 or TypeID == 8029006 or TypeID == 8039006 or TypeID == 8049006 then
		    type2 = 5
...
function x_OnSoulCrystal_GetSkill(TypeID, ZiZhi)
	...
	if TypeID == 8220401 or TypeID == 8220403 or (TypeID >= 8220501 and TypeID <= 8220599) then
		if TypeID == 8220524 then
			return 2602501
		else
			if index == 1 then
...
function x_OnSoulCrystal_GetAbility(MapID, InstanceID, TypeID, FirstTime, ZiZhi, Pct1, Pct2, RoleID, ID1, ID2, ID3, ID4, ID5, ID6, ID7, ID8) --RoleID有可能是-1或者0
	...
	if TypeID == 8220524 then
		return 104310,104310,205310,105310,105310,206310,313310,313310
	else
		if FirstTime == 1 then
			local tbl = {}
			local maxnum = 0
